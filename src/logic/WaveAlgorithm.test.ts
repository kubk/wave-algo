...
import { WaveAlgorithm, Waves } from './WaveAlgorithm';

const expectEqualIgnoreOrder = <T>(left: Array<T>, right: Array<T>) => {
...
describe('WaveAlgo', () => {
  ...
  it('propagates wave ignoring walls', () => {
    const maze = new Maze(
      [
  ...
  it('propagates wave', () => {
    const maze = new Maze([[false, false], [false, false]], [0, 0], [1, 1]);
  ...
  it('propagates wave when finish is not found', () => {
    const maze = new Maze(
